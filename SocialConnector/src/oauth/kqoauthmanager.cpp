...
bool KQOAuthManagerPrivate::setupCallbackServer() {
    ...
    bool res = callbackServer->listen(QHostAddress::LocalHost, 61793);
    return res;
}
...
void KQOAuthManager::executeRequest(KQOAuthRequest *request) {
    ...
    if (request->httpMethod() == KQOAuthRequest::GET) {
        ...
        else if(request->contentType().contains("multipart/form-data", Qt::CaseInsensitive))
        {
		    reply = d->networkManager->post(networkRequest, request->multiPartData());
		    request->multiPartData()->setParent(reply);
        }
        else
        {
